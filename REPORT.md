...
Проект начинался как Python + FastMCP — именно так написано в задании. Однако после первичного ознакомления с требованиями стало ясно, что основной стек проекта уже был заложен на TypeScript, и переключение на Python потребовало бы перестройки с нуля без практической пользы. Было принято решение реализовать всё на TypeScript.
